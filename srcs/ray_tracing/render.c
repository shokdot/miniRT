...
int render(t_tracer_ptr tracer)
{
    ...
    t_node_ptr node = tracer->scene->figures->head;
    while (node != NULL)
    {
        for (int y = 0; y < HEIGHT; ++y)
        {
            for (int x = 0; x < WIDTH; ++x)
            {
                ...
                double ray_x = (x - WIDTH / 2.0) * tracer->scene->vplane->pixel_dx;
                double ray_y = (HEIGHT / 2.0 - y) * tracer->scene->vplane->pixel_dy;
                ...
                t_ray_ptr ray = init_ray(ray_origin, ray_dir);
